    ...
    <ul class="nav nav-tabs nav-border-top nav-border-top-info mb-3" role="tablist" id="myTab">
        ...
        <li class="nav-item" role="presentation">
            <a class="nav-link" data-bs-toggle="tab" href="#konfirmasi_reservasi" role="tab" aria-selected="true">
                Konfirmasi Reservasi @if (count($reservasi_mobil->whereIn('id_status', [4, 3, 14, 13])) > 0)
                <span class="badge bg-danger rounded-circle">{{ count($reservasi_mobil->whereIn('id_status', [4, 3, 13, 14])) }}</span>
                @endif
            </a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link" data-bs-toggle="tab" href="#riwayat" role="tab" aria-selected="false" tabindex="-1">
                Riwayat @if(count($reservasi_mobil->where('id_status', 5)->where('id_pengantaran', '!=', 3)) > 0)
                <span class="badge bg-danger rounded-circle">{{ count($reservasi_mobil->where('id_status', 5)->where('id_pengantaran', '!=', 3)) }}</span>
            @endif
            </a>
        </li>
        <li class="nav-item" role="presentation">
            <a class="nav-link" data-bs-toggle="tab" href="#voucher" role="tab" aria-selected="false" tabindex="-1">
                Voucher @if(count($reservasi_mobil->where('id_pengantaran', 3)) > 0)
                <span class="badge bg-danger rounded-circle">{{ count($reservasi_mobil->where('id_pengantaran', 3)) }}</span>
                @endif
            </a>
        </li>
    </ul>
            ...
            <div>
                <div class="table-responsive">
                    <table class="table align-middle table-nowrap mb-0" id="konfirmasi_reservasiTable">
                        ...
                        <tbody>
                            @foreach ($reservasi_mobil->whereIn('id_status', [4, 3, 13, 14]) as $row)
                                <tr>
            ...
            <div>
                <div class="table-responsive">
                    <table class="table align-middle table-nowrap mb-0" id="riwayatTable">
                        ...
                        <tbody>
                            @foreach ($reservasi_mobil->where('id_status', 5)->where('id_pengantaran', '!=', 3) as $row)
                                <tr>
                                    <th scope="row">{{ $loop->iteration }}</th>
                                    <td><a href="{{ url('reservasi-mobil/show', $row->kode_pemesanan) }}" class="fw-medium">{{ $row->user->name }}</a></td>
                                    <td>
