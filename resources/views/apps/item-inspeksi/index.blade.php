                    ...
                    <div class="card-header align-items-center d-flex">
                        ...
                        <div class="flex-shrink-0">
                            <div class="form-check form-switch form-switch-right form-switch-md">
                                <a href="{{ url('item-inspeksi/create') }}" class="btn btn-info">Tambah Data</a>
                            </div>
                        </div>
                    </div><!-- end card header -->
                        ...
                        <div>
                            <div class="table-responsive">
                                <table class="table align-middle table-nowrap mb-0" id="myTable">
                                    ...
                                    <tbody>
                                        @foreach ($itemInspeksi as $row)
                                            <tr>
                                                <th scope="row"><a href="#" class="fw-medium">{{ $loop->iteration }}</a></th>
                                                <td>{{ $row->item }}</td>
                                                <td>{{ $row->tipe_inspeksi }}</td>
                                                <td>@if($row->jenis_kendaraan == 1) Non Ambulance @else Ambulance @endif</td>
                                                <td>{{ $row->created_at }}</td>
                                                <td>
                                                    <a href="{{ route('item-inspeksi.edit', $row->id) }}" class="btn btn-info btn-sm edit-btn"><i class="ri-pencil-fill fs-16"></i></a>
                                                    ...
                                                    @include('apps.item-inspeksi.components.modal-delete')
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
